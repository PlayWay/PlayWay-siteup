...
app.use(session({
    secret: 'masjdaklsjdn1m2n,±JJ!J!nm,nczasjd2@',
    resave: false,
...
app.post('/order', function (req, res) {
    let mailOptions = {
        from: 'Заявка Siteupers <siteinformingservice@gmail.com>',
        to: 'caspanch@gmail.com',
        subject: 'Заявка Siteupers',
        html: '<p>Заявка Siteupers</p>'
    };

    mailOptions.html += '<p>Name: ' + req.body.name + '</p>';
    mailOptions.html += '<p>Email: ' + req.body.email + '</p>';
    mailOptions.html += '<p>Phone: ' + req.body.phone + '</p>';
    mailOptions.html += '<p>Message: ' + req.body.message + '</p>';

    if(req.body.steps.one !== null) {
        mailOptions.html += '<p>Step One: ' + req.body.steps.one + '</p>';
    }

    if(req.body.steps.two.length > 0) {
        mailOptions.html += '<p>Step Two:</p>';
        mailOptions.html += '<ul>';
        req.body.steps.two.map((product) => {
            mailOptions.html += '<li>' + product + '</li>';
        });
        mailOptions.html += '</ul>';
    }

    if(req.body.steps.three.length > 0) {
        mailOptions.html += '<p>Step Three:</p>';

        mailOptions.html += '<ul>';
        req.body.steps.three.map((product) => {
            mailOptions.html += '<li>' + product + '</li>';
        });
        mailOptions.html += '</ul>';
    }
